...
//
// MartySama 5.8 profile: Uses boost::unordered_map for m_entryMap to match
// the Boost-based CEterFileDict (boost::unordered_multimap) used throughout
// the MartySama codebase.
class CVpkPack
{
public:
    CVpkPack();
...
private:
    ...
    // Boost unordered_map for CRC32 hash -> entry index (matches MartySama's Boost usage)
    boost::unordered_map<DWORD, int> m_entryMap;

//
// VpkCrypto.cpp — FliegeV3 Profile
//
// Standalone cryptographic implementation using OpenSSL 3.x.
// System-agnostic — identical to the 40250 profile since VpkCrypto
// has zero dependencies on the Metin2 source tree.
//
...
int VpkCrypto::ZstdDecompress(const unsigned char* src, int srcLen,
                               unsigned char* dst, int originalSize)
{
    size_t result = ZSTD_decompress(dst, static_cast<size_t>(originalSize),
                                     src, static_cast<size_t>(srcLen));
...
int VpkCrypto::BrotliDecompress(const unsigned char* src, int srcLen,
                                 unsigned char* dst, int originalSize)
{
    size_t outSize = static_cast<size_t>(originalSize);
    BrotliDecoderResult result = BrotliDecoderDecompress(
        static_cast<size_t>(srcLen), src,
        &outSize, dst
    );
